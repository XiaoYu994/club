<template>
  <view class="edit-recruitment-container pageBg">
    <view class="status-bar" :style="{ height: statusBarHeight + 'px' }"></view>
    <custom-nav-bar title="编辑招新" :showBack="true" @backClick="goBack"></custom-nav-bar>
    <scroll-view scroll-y class="form-container">
      <!-- 基本信息 -->
      <view class="form-section">
        <view class="section-title">基本信息</view>
        <view class="form-item">
          <text class="form-label">招新配置：</text>
          <view class="form-picker">{{ selectedConfig ? selectedConfig.name : '加载中...' }}</view>
        </view>
        <view class="form-item">
    ...
    <view class="form-actions">
      ...
      <button class="submit-btn" @tap="updateRecruitment">保存更新</button>
    </view>
  </view>
</template>
...
onMounted(async () => {
  ...
  // 如果没有recruitmentId，返回上一页
  if (!recruitmentId.value) {
    uni.showToast({ title: '招新ID不能为空', icon: 'none' })

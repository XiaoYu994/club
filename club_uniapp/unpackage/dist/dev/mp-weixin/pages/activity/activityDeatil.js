...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const showApplyBtn = common_vendor.ref(true);
      ...
      if (currentPage && currentPage.options) {
        ...
        if (forceRefresh || !currentPage.options.itemData) {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:444", "解析活动数据失败:", e);
            loadActivityDetail();
          }
        }
        getUserInfo();
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:465", "获取页面参数失败");
    ...
    const handleCheckInSuccess = async (data) => {
      if (data && data.activityId && data.activityId.toString() === id.value.toString()) {
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:488", "收到签到成功事件，开始刷新活动详情");
        await refreshActivityData();
        await checkApplyStatus(false);
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:496", "活动详情刷新完成");
      }
    };
      ...
      try {
        ...
        if (res.code === 200 && res.data) {
          ...
          if (oldPoster !== activityDetail.value.poster) {
            common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:522", "海报已更新:", activityDetail.value.poster);
          }
          if (oldJoinCount !== activityDetail.value.joinCount || oldViewCount !== activityDetail.value.viewCount) {
            common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:527", "数据已更新:", {
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:537", "刷新活动数据失败:", error);
      }
    };
    ...
    const loadActivityDetail = async () => {
      if (!id.value) {
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:559", "活动ID不存在");
        common_vendor.index.showToast({
          title: "活动ID不存在",
          icon: "none"
        });
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:594", "加载活动详情失败:", error);
        common_vendor.index.showToast({
          title: "网络异常，请稍后重试",
          icon: "none"
        });
    ...
    const parseFormFieldsFromData = () => {
      try {
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:607", "解析表单字段，活动详情:", JSON.stringify(activityDetail.value));
        if (activityDetail.value && activityDetail.value.forms) {
          ...
          common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:612", "活动没有自定义表单，使用默认表单");
          formFields.value = utils_activity.parseFormFields(null);
        }
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:617", "解析表单字段失败:", e);
        formFields.value = utils_activity.parseFormFields(null);
      }
    };
    ...
    const checkApplyStatus = async (showLoading = true) => {
      try {
        ...
        if (res.code === 200) {
          if (res.data) {
            ...
            try {
              ...
              common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:696", "解析表单数据失败:", e);
          ...
          showApplyBtn.value = !hasApplied.value && utils_activity.getActualStatus(activityDetail.value) === 1;
          refreshActivityData();
        } else {
          common_vendor.index.showToast({
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:720", "检查报名状态失败:", error);
        hasApplied.value = false;
        applyInfo.value = {};
        parsedForms.value = {};
        common_vendor.index.showToast({
    ...
    const showApplyForm = () => {
      ...
      if (formFields.value && formFields.value.length > 0) {
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:847", "填充前的表单字段:", JSON.stringify(formFields.value));
        formFields.value = utils_activity.fillUserInfoToForm(formFields.value, userInfo.value);
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:852", "填充后的表单字段:", JSON.stringify(formFields.value));
      } else {
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:854", "表单字段为空，无法显示报名表单");
        common_vendor.index.showToast({
          title: "表单加载失败，请重试",
          icon: "none"
        });
    ...
    const submitApply = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:958", "提交报名失败:", error);
        common_vendor.index.hideLoading();
    ...
    const showQRCode = async () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:1134", "生成签到码错误:", error);
        common_vendor.index.showToast({
          title: "网络异常，请稍后重试",
          icon: "none"
        });
    ...
    const formatExpireTime = (timestamp) => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:1151", "格式化时间错误:", error);
        return "时间格式错误";
      }
    };
    const closeQRCode = () => {
    ...
    const onCheckboxChange = (index, e) => {
      ...
      common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:1232", `选项变更 - 字段: ${formFields.value[index].label}, 值: ${formFields.value[index].value}`);
    };
    ...
    const handleImageError = (e) => {
      common_vendor.index.__f__("error", "at pages/activity/activityDeatil.vue:1249", "二维码图片加载失败:", e);
      showBackupQRCode.value = true;
      if (applyInfo.value && applyInfo.value.qrCodeUrl) {
        common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:1255", "尝试使用备用方式显示二维码:", applyInfo.value.qrCodeUrl.substring(0, 50) + "...");
      }
    };
    const handleImageLoad = () => {
      common_vendor.index.__f__("log", "at pages/activity/activityDeatil.vue:1261", "二维码图片加载成功");
